...
## 최신 체크포인트 — 2026-09-26 S2 기획·자산 인계

**새 작업은 [S2 시작 문서](s2/README.md)부터 읽는다.** 사용자는 Figma S2 전체 적용을 원하며, 내보내기 제한 뒤에는 확보 이미지로 진행하도록 했다. 원본 17종·여백 정리 8종·이미지 레이어 13종을 `design/s2/`에 보존했다. 기획과 기존 34개 경로/시안 45개 프레임 매핑, 현재 APK/변경 예상 비교 6개는 `docs/redesign/s2/`에 있다.

현재는 문서·자산 준비 단계다. S2 앱 소스 변경·빌드·배포는 하지 않았다. 비교 HTML은 최종 승인 시안이 아니며 작은 PNG는 해상도 기준이 아니다. 확정·제안·미정 구분과 다음 순서는 [결정 기록](s2/DECISIONS.md), [진행 상태](s2/STATUS.md)를 따른다. 아래 단계 기록은 기존 앱 작업의 증거이며 S2 완료 증거가 아니다. 이 인계 정리 자체로 새 구현이나 이전 장기 목표를 시작하지 않는다.
