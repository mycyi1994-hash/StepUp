...
2026-09-25 제작. **효과음 26개 + 선택형 배경 루프 3개 = 원본 사운드 29개**다. 모든 파일은 직접 합성한 원본이며 외부 샘플이나 음원을 사용하지 않았다.

같은 날 앱 연결까지 진행했다. 29개 원본을 `app/src/main/res/raw`에 복사했고, 공통 조작·착용·확정 결과·러닝·뽑기 진입 18개는 앱 이벤트에 연결했다. 뽑기 결과 관련 8개는 `web/assets/sounds`에도 복사해 거래 영수증 확인/취소/실패 이벤트에 연결했지만, 실제 DApp 주소와 계약 배포가 완료되기 전에는 사용자가 들을 수 없다. 배경 루프 3개는 환경 설정에서 별도로 켜야 하며 앱이 전면에 있고 다른 음악이 없을 때만 재생한다. [파일별 적용표](../../../docs/redesign/MEDIA-APPLICATION-2026-09-25.csv)에 원본·앱·웹 경로와 상태를 적었다.
