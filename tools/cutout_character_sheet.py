...
사용: python3 tools/cutout_character_sheet.py <칸 그림> <출력 png> [허용 거리=24] [다리 사이 시작=0.74]
  환경 변수(모두 선택)
    CUTOUT_NOCROP=1   결과를 캐릭터 크기로 자르지 않는다(뒤에서 칸 좌표로 더 손볼 때)
    CUTOUT_BG2D=1     바탕을 줄이 아니라 면으로 추정한다(칸 안에 밝은 판 · 비네팅이 있는 시트)
    CUTOUT_KEEP=0.03  가장 큰 덩어리의 그 비율 이상인 덩어리도 남긴다(몸과 끊긴 신발, 모자 · 상의 · 반바지 상품)
다리 사이 시작 값이 1 이상이면 캐릭터가 아닌 상품 그림으로 보고 발 · 바닥 처리를 건너뛴다.
"""
