...
adb pull /sdcard/Android/data/com.stepup.android/files/experience-qa/. qa-captures/ || status=1
adb logcat -d -s AndroidRuntime > qa-captures/android-crashes.txt || true
captures=$(find qa-captures -maxdepth 1 -name '*.png' | wc -l)
if [ "$captures" -lt 455 ]; then
  echo "Expected 455 Android screenshots, found $captures"
  status=1
fi
exit "$status"
