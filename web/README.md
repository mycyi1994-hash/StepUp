...
| `/wallet.html` | 지갑 — 2단계 인증 · 지갑 연결 · 보너스 뽑기 · SUP/신발 꺼내기(가스비 대납) · 앱으로 넣기. 앱에서 여는 길(`…/wallet.html#t=<로그인 토큰>` — # 뒤는 서버로 가지 않고, 페이지가 읽자마자 주소창에서 지운다)은 **앱 전환(5단계)에서 연결한다 — 아직 앱에 버튼이 없다.** 그 전까지는 구글 로그인으로 들어온다. 설정은 `wallet-config.js`, 빌드는 `npm run build:wallet`. |
